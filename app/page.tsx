  ...
  return (
    <main id="main-content" tabIndex={-1} aria-busy={transcoding} className="relative min-h-screen overflow-hidden bg-transparent text-slate-100">
      ...
      <div className="mx-auto max-w-[1440px] px-4 py-4 sm:px-6 lg:px-8">
        <motion.header {...revealProps} className="sticky top-3 z-20 mb-7 flex items-center justify-between rounded-2xl border border-white/10 bg-[#111a30]/90 px-4 py-3 shadow-2xl shadow-black/20 backdrop-blur-xl">
        ...
        <AnimatePresence>
        {updateWorker && <motion.section role="status" {...collapseProps} className="mb-5 overflow-hidden rounded-2xl border border-indigo-300/25 bg-indigo-300/10">
        ...
        <section className="mb-7 grid gap-5 lg:grid-cols-[220px_1fr]">
          <motion.aside {...(reduceMotion ? { initial: false } : { initial: { opacity: 0, x: -16 }, animate: { opacity: 1, x: 0 }, transition: { delay: .08, duration: .4, ease: [0.22, 1, 0.36, 1] } })} className="hidden rounded-2xl border border-white/10 bg-[#111a30] p-3 lg:block">
          ...
          <motion.div id="studio-content" {...(reduceMotion ? { initial: false } : { initial: { opacity: 0, y: 18 }, animate: { opacity: 1, y: 0 }, transition: { delay: .12, duration: .45, ease: [0.22, 1, 0.36, 1] } })}>
            ...
            <div className="mb-5 h-1.5 overflow-hidden rounded-full bg-white/10"><motion.div className="h-full rounded-full bg-gradient-to-r from-cyan-300 to-indigo-400" animate={{ width: `${((step + 1) / steps.length) * 100}%` }} transition={reduceMotion ? { duration: 0 } : { type: 'spring', stiffness: 170, damping: 25 }} /></div>
